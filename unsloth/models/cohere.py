...
def CohereModel_fast_forward(
    ...
    *args, **kwargs,
    ...
    if isinstance(self.config.torch_dtype, str):
        # It's a string, so convert the string to a torch.dtype
        dtype_str = self.config.torch_dtype
        torch_dtype = getattr(torch, dtype_str)
        inputs_embeds = inputs_embeds.to(torch_dtype)
    else:
        # Not a string, assuming it's already a torch.dtype or similar
...
class CohereRotaryEmbedding(torch.nn.Module):
...
class FastCohereModel:
    ...
    def pre_patch():
        ...
        CohereModel          .forward = CohereModel_fast_forward
        ...
        # transformers.models.llama.modeling_llama.CohereRotaryEmbedding = CohereRotaryEmbedding
        # transformers.models.llama.modeling_llama.CohereLinearScalingRotaryEmbedding = CohereLinearScalingRotaryEmbedding
        return
    pass
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if model_patcher is None: model_patcher = FastCohereModel
        ...
        # RoPE scaling
        model_config = CohereConfig.from_pretrained(model_name, token = token)
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        model = FastCohereModel.patch_peft_model(model, use_gradient_checkpointing)
